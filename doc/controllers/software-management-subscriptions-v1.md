...
try {
  const response = await softwareManagementSubscriptionsV1Controller.getAccountSubscriptionStatus(account);
...
try {
  const response = await softwareManagementSubscriptionsV1Controller.getAccountLicenseStatus(
    account,
    startIndex
  );
